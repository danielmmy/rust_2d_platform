...
**looks up** and **Down** **crouches** (each with its own pose). Menus still confirm with
`Enter` / `Space` / `Z`.
...
- `player.png` is **6×6** — a **side-profile** character facing right (the
  ...
  (hold Down), **5 = look-up** (hold Up). Driven by state — damage → jump (airborne) →
  walk (moving) → crouch/look-up (still) → idle; the jump plays **once across the arc**
  (launch → apex → fall), keyed to velocity. Rows 4–5 are generated from the idle row by
  [`tools/gen_player_poses.py`](tools/gen_player_poses.py) (a squash / stretch) — rough
  placeholders to redraw by hand; the first four rows are left untouched.
...
- **2026-06-28** — **Controls + moveset.** Jump is now a **dedicated button** (`Space` /
  ...
  **Down** **crouches**, each with its own pose (player sheet extended to **6×6**, new rows
  ...
  Added a **Hollow-Knight pogo** ([`combat`](src/combat.rs)): an airborne **Down + attack**
  slashes downward and bounces you up off any enemy, the boss, or a hazard (e.g. spikes),
  refreshing the air-jump so bounces chain.
