...
- **2026-06-29** — **New hero art (blocky 8-bit).** Redesigned the player in a minimal,
  flat-block Celeste-ish style — simple shapes, lots of frames for fluid motion. The sheet is
  now **6×11** (`assets/sprites/player.png`): the original eight rows plus **dash**, **attack**,
  and **pogo**, all wired in [`anim.rs`](src/anim.rs) (the attack/pogo poses key off the live
  [`combat::Sword`](src/combat.rs) swing, dash off [`JumpState::dashing`](src/player.rs)). The
  character is taller — the hitbox is now **~1.7 tiles standing / 1 tile crouched**
  (`PLAYER_HALF`/`CROUCH_HALF`), rendered at 2× from 40px frames. Art is generated by
  [`art_samples/hero_gen.py`](art_samples/hero_gen.py) (`bake_sheet` writes the sheet; the same
  script emits the review GIFs/contact sheet). The old sheet is kept at
  `art_samples/player_old.png`.
